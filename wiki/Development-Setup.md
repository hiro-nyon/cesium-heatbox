<!-- Generated from docs/development-setup.md by npm run wiki:sync. Edit the canonical source, not this page. -->
...
- `feature/*` / `fix/*`: Short-lived branches created from `next`

Open pull requests against `next`. Stable releases are promoted from `next` to `main` after review.
...
- `main`: 安定リリース用ブランチ
- `next`: 次期リリースの統合ブランチ
- `feature/*` / `fix/*`: `next` から作成する短命ブランチ

Pull Request は `next` 向けに作成します。安定リリース時はレビュー後に `next` から `main` へ反映します。
