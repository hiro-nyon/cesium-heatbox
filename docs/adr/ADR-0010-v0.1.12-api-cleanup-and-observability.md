...
- [ ] 型定義更新
  - [ ] `types/`配下またはJSDocの`@typedef`で新オプション定義（`emulationScope` 等）
  - [ ] `outlineRenderMode`/`outlineWidthPreset(thin|medium|thick|adaptive)` の文字列リテラル型提供
...
- [ ] ブラウザ互換性テスト
- [ ] ドキュメント整合性確認
- [ ] （追加）v0.1.12で作成/更新したドキュメントのバイリンガル整合確認（日本語/English）
  - [x] README（日英併記の維持）
  - [ ] MIGRATION.md（新規作成・日英対応）
  - [ ] ADR-0010（主要節に英語説明の併記）
  - [x] Wiki（JSDoc→Markdown変換で日英セクション整備）
  - [ ] examples/advanced 配下のREADME・ガイド（日英併記に更新）
