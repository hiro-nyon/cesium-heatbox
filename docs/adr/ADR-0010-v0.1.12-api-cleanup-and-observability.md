...
**Implementation State**: Implemented — the performance overlay (`src/utils/performanceOverlay.js`), option
profiles (`src/utils/profiles.js`), unified `pitchDegrees`/`headingDegrees` naming, and `emulationScope` are
all current.
