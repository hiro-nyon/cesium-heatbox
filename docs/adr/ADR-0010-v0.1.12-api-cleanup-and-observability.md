...
- [ ] `MIGRATION.md`作成
  ...
  - [ ] コード例による移行手順（双方向例を含む）
  - [ ] よくある移行パターンと落とし穴
- [ ] Phase 1 で延期した同期作業の一括実施（明示）
  - [ ] README の更新（新API、`emulationScope`、プリセット新名、`getEffectiveOptions` 反映）
  - [ ] JSDoc の再生成（docs/api）と typedef の更新（`outlineRenderMode`/`emulationScope`/プロファイル/オーバーレイ）
  - [ ] Examples（basic/advanced）の新API対応（`pitchDegrees/headingDegrees`、新プリセット、`emulationScope` の使用例）
- [ ] 型定義更新
