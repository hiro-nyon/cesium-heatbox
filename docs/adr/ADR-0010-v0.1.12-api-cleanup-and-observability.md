...
- 文字列 互換:
  ...
  - `'topn'` → `outlineRenderMode: 'emulation-only'`, `emulationScope: 'topn'`
  - `'non-topn'` → `outlineRenderMode: 'emulation-only'`, `emulationScope: 'non-topn'`
- 未設定時の既定値: `'standard'`
...
補足: `emulationScope` は `outlineRenderMode !== 'emulation-only'` の場合は無視される。将来的に（v0.2.0）`emulationScope` を削除する方針の場合は本ADRの Migration にスケジュールを明示する。
...
- [ ] examples/basic, examples/advanced の動作確認
- [ ] 後方互換性テスト（該当する場合）

### Phase 1: APIクリーンアップ実装（Week 1-2）
- [ ] オプション命名統一の実装
  - [ ] `fitViewOptions.pitch/heading` → `pitchDegrees/headingDegrees`
  - [ ] 新名前優先、旧名前は警告表示で対応
- [ ] Resolver系削除の実装
  - [ ] `adaptiveParams`システムとの統合
  - [ ] 削除予定オプションの警告表示
- [ ] `outlineRenderMode`統一の実装
  - [ ] `outlineEmulation`（boolean/文字列）からの移行ロジック
  - [ ] `emulationScope` 追加と互換マッピング
 - [ ] プリセット名の統一
  - [ ] レガシー名（`uniform`/`adaptive-density`/`topn-focus`）のマッピングと警告
 - [ ] 定数/型/JSDoc/Examplesの同期更新

### Phase 2: 観測可能性機能追加（Week 2-3）
- [ ] パフォーマンスオーバーレイUI作成
  - [ ] リアルタイム統計表示
  - [ ] ON/OFF切り替え機能
  - [ ] `examples/advanced/`への統合
- [ ] プロファイル機能実装
  - [ ] プリセット定義と適用ロジック
  - [ ] `validateAndNormalizeOptions`でのマージ処理
  - [ ] `Heatbox.listProfiles()`（静的）/ `heatbox.getEffectiveOptions()`（インスタンス）APIの実装
- [ ] ベンチマーク改善
  - [ ] `tools/benchmark.js`の出力形式改善
  - [ ] 統計分析機能追加

### Phase 3: ドキュメント・移行支援（Week 3-4）
- [ ] `MIGRATION.md`作成
  - [ ] オプション対応表作成（旧→新マッピング辞書）
  - [ ] コード例による移行手順（双方向例を含む）
  - [ ] よくある移行パターンと落とし穴
- [ ] 型定義更新
  - [ ] `types/`配下またはJSDocの`@typedef`で新オプション定義
  - [ ] `outlineRenderMode`の文字列リテラル型提供
  - [ ] プロファイル名とオーバーレイ設定の型安全性確保
- [ ] README・Wiki同期更新
  - [ ] `tools/wiki-sync.js`の対象ページリストに`MIGRATION.md`追加
  - [ ] API参照の新オプション反映
- [ ] examples更新（新API対応）
  - [ ] `examples/advanced/`にパフォーマンスオーバーレイ有効化サンプル追加
