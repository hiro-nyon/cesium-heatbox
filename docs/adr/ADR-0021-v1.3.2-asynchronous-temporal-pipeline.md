...
**Related**: [ADR-0018](ADR-0018-v1.2.0-time-dependent-data.md) (v1.2.0 temporal PoC), [ADR-0020](ADR-0020-v1.3.0-incremental-entity-rendering.md) (incremental rendering and camera-aware planning), [ADR-0019](ADR-0019-v1.3.7-current-runtime-architecture.md) (current runtime architecture), [ROADMAP v1.3.1 / v1.3.2](../../ROADMAP.md)
...
ADR-0018 established the temporal architecture that is still in force: `temporal` options, a `TimeController` bound to `viewer.clock.onTick`, and a `TimeSlicer` that resolves a `JulianDate` to a data slice. None of that was reversed. What changed is that four capabilities ADR-0018 **explicitly deferred** were subsequently built in the v1.3.x line. The exact patch/minor delivery points changed, as the comparison below records:

ADR-0018 が確立した時系列アーキテクチャは現在も有効である — `temporal` オプション群、`viewer.clock.onTick` に結線された `TimeController`、`JulianDate` をデータスライスへ解決する `TimeSlicer`。これらはいずれも撤回されていない。変わったのは、ADR-0018 が **明示的に先送りした** 4つの機能がその後 v1.3.x 系で実装されたことである。正確な patch/minor の提供時点は変更されており、その差を以下に記録する。
...
Two alternatives ADR-0018 *rejected for v1.2.0* were therefore later adopted in the anticipated v1.3.x line, although not at every exact version named in ADR-0018. That is an evolution of a decision, not a reversal of one, so ADR-0018 remains `Accepted` and this ADR records the additions. ADR-0018's Alternative 3 (3D Tiles integration) was deferred to v2.0 and remains unimplemented.

すなわち ADR-0018 が *v1.2.0 では却下した* 代替案2件が、予告された v1.3.x 系で後に採用された。ただし ADR-0018 が記したすべての正確なバージョンどおりではない。これは決定の撤回ではなく発展であるため、ADR-0018 は `Accepted` のまま維持し、本ADRが追加分を記録する。ADR-0018 の Alternative 3（3D Tiles 統合）は v2.0 へ先送りされ、現在も未実装である。
...
The synchronous **slice-resolution branch** remains for the common case. The asynchronous resolution branch is entered **only** when `temporal.dataSource` or `temporal.useWorker` is configured. Both branches nevertheless share the post-resolution machinery introduced after v1.2.0, including `updateValues`, single-slot update coalescing, and camera-triggered refresh. A `null` entry is deliberately allowed through the change-detection guard so that `outOfRangeBehavior` can still run.

一般的なケースでは同期的な **スライス解決分岐** が残る。非同期解決分岐に入るのは `temporal.dataSource` または `temporal.useWorker` が設定されている場合 **のみ** である。ただし両分岐は、v1.2.0以後に導入された `updateValues`、単一スロットの更新coalescing、camera起点refreshを共有する。`null` エントリは意図的に変更検知ガードを通過させ、`outOfRangeBehavior` が動作できるようにしている。
...
`deactivate()` also increments `_asyncTickRequestId`, so an in-flight slice resolution is discarded before it can start a Heatbox update. This does **not** cancel an `updateValues()` call that had already started; the current implementation has no destroy epoch or post-`await` destroyed guard for that case.

`deactivate()` も `_asyncTickRequestId` をインクリメントするため、進行中のスライス解決結果は Heatbox 更新を開始する前に破棄される。ただし、既に開始済みの `updateValues()` はキャンセルされない。現実装には、この場合に対する destroy epoch や `await` 後の destroyed guard は無い。
...
Items are paired by array index first. The `id` search is only a fallback when the item at that index is missing; it does not detect a reordered same-length array. Stable spatial identity therefore requires the caller to keep entity ordering consistent across slices (or to leave holes that allow the fallback to run).

項目の対応付けはまず配列インデックスで行う。`id` 検索はそのindexのitemが欠けている場合だけのfallbackであり、同じ長さの配列の並び替えは検出しない。したがって空間identityを安定させるには、呼び出し側がslice間のentity順序を維持する必要がある（またはfallbackが動く欠損位置を残す必要がある）。

### 8. Asynchronous `dataSource` (lazy loading) / 非同期 `dataSource`（遅延ロード）

`temporal.dataSource` is a caller-supplied function invoked with `(currentTime, { loadedEntries, timeRange })`. `TimeSlicer.getEntryAsync` tries a direct lookup first, and only on a miss starts a load — with one **global single-flight guard** so concurrent ticks share one in-flight request. The guard is not keyed by requested time: if a second time misses while a first time's load is pending, it waits for that first load and does not automatically issue its own load if the first result does not cover it.

`temporal.dataSource` は呼び出し側が与える関数で、`(currentTime, { loadedEntries, timeRange })` で呼ばれる。`TimeSlicer.getEntryAsync` はまず直接検索を試み、ミス時にのみロードを開始する。時刻でkeyed化されていない単一の **global single-flightガード** により、同時tickは1件の進行中requestを共有する。2番目の時刻が最初の時刻用load中にmissすると、そのloadを待つが、結果が2番目の時刻を含まなくても自分用loadを自動発行しない。
...
Merged entries go through the same normalisation, sorting, and overlap resolution as constructor-supplied data, so a lazily loaded slice cannot violate those eager-path entry invariants. `_mergeLoadedEntries` invalidates `TimeSlicer._globalStatsCache`, but the live global-classification domain used for rendering is stored separately in `Heatbox._globalStats` and is not recomputed after a lazy merge. A domain computed from initial partial data can therefore persist; with no initial entries, global scope can effectively fall back to per-time statistics.

マージされたエントリは、コンストラクタ供給データと同じ正規化・ソート・重複解消を通るため、eager経路のentry不変条件は維持される。`_mergeLoadedEntries` は `TimeSlicer._globalStatsCache` を無効化するが、実描画のglobal分類domainは別の `Heatbox._globalStats` に保持され、lazy merge後に再計算されない。そのため初期partial dataから計算したdomainが残り得る。初期entryが無い場合、global scopeは実質的にper-time統計へfallbackし得る。
...
`destroy()` rejects requests currently present in the pending map, terminates the current worker, and revokes the currently tracked object URL. `TimeController.deactivate()` calls `TimeSlicer.destroy()`, which calls it, so this cleanup is on the same path as listener removal. There are important gaps: a synchronous `postMessage()` throw leaves its pending-map entry behind; construction/runtime failure does not revoke the current Blob URL before a later creation can overwrite the handle; and neither `TimeSlicer` nor the bridge has a destroyed guard, so a pending `dataSource` can resume after teardown and create another worker.

`destroy()` はpending mapにその時点で存在するrequestをrejectし、現在のworkerをterminateし、現在追跡中のobject URLをrevokeする。`TimeController.deactivate()` が `TimeSlicer.destroy()` を呼び、それがこれを呼ぶため、この解放はlistener解除と同一経路上にある。ただし重要な不足がある。同期的な `postMessage()` throwではpending map entryが残り、生成／runtime failureでは次の生成がhandleを上書きする前に現在のBlob URLをrevokeせず、`TimeSlicer`とbridgeのどちらにもdestroyed guardが無いためpending中の `dataSource` が破棄後に再開してworkerを再生成し得る。
...
- **In the generated worker today**: numeric interpolation between two slice payloads (`interpolate` task). A `stats` branch exists, but it throws because the generated script omits `calculateTemporalValueStats`.
  **現在の生成worker内**: 2つのスライスペイロード間の数値補間（`interpolate` タスク）。`stats` 分岐は存在するが、生成scriptが `calculateTemporalValueStats` を含めないためthrowする。
...
The worker protocol defines a `stats` branch, and `TimeSlicer.calculateGlobalStatsAsync()` is written to request it. However, the generated worker script serialises `calculateTemporalStats()` without serialising its dependency `calculateTemporalValueStats()`. A real `stats` message therefore fails with `ReferenceError: calculateTemporalValueStats is not defined`. Existing tests use worker doubles and do not execute the generated Blob script. In addition, **`calculateGlobalStatsAsync()` is not called anywhere in `src/`** — only from tests. The live global-scope path is `TimeController._activateWithAsyncStats` → `TimeSlicer.calculateGlobalVoxelStats()`, which runs a full `DataProcessor` classification per slice **on the main thread** (it is `async`, but not offloaded). So:

worker プロトコルには `stats` 分岐があり、`TimeSlicer.calculateGlobalStatsAsync()` はそれを要求するよう書かれている。しかし生成worker scriptは `calculateTemporalStats()` を直列化する一方、その依存先 `calculateTemporalValueStats()` を含めていない。このため実際の `stats` message は `ReferenceError: calculateTemporalValueStats is not defined` で失敗する。既存テストはworker doubleを用いており、生成されたBlob scriptを実行していない。さらに **`calculateGlobalStatsAsync()` は `src/` のどこからも呼ばれていない** — テストからのみ呼ばれる。実際の global scope 経路は `TimeController._activateWithAsyncStats` → `TimeSlicer.calculateGlobalVoxelStats()` であり、これはスライスごとに完全な `DataProcessor` 分類を **メインスレッドで** 実行する（`async` ではあるが offload されていない）。したがって:

- the worker is wired into the live path for **interpolation only**;
  worker が実経路に結線されているのは **補間のみ** である。
- the `stats` branch is present but currently fails in the generated worker, and no production path reaches it.
  `stats` 分岐は存在するが生成worker内では現在失敗し、かつ production 経路からは到達しない。

The v1.3.2 CHANGELOG describes `useWorker` as offloading "補間と時系列統計の前処理" — interpolation *and* temporal statistics preprocessing. Only interpolation is currently functional in the generated worker. The statistics half is neither wired into production nor executable until the missing helper is included. Recorded here so the discrepancy is documented rather than propagated.

CHANGELOG v1.3.2 は `useWorker` を「補間と時系列統計の前処理」を offload するものと記述している。生成workerで現在機能するのは補間のみである。統計側はproductionへ未結線であり、不足helperを含めるまで実行もできない。この不一致を伝播させず記録するために、ここに明記する。
...
`activate()` binds both listeners through idempotent helpers; `deactivate()` removes those registered listeners and destroys the current slicer worker, subject to the asynchronous gaps described below:

`activate()` は冪等なhelper経由で両listenerを束縛し、`deactivate()` は登録済みlistenerを解除して現在のslicer workerを破棄する。ただし後述の非同期上の不足がある。
...
Cesium's `Event.addEventListener` returns a remover function, and both removers are retained rather than re-deriving the bound handler — necessary because `this._onTick.bind(this)` creates a new function object each call and could not otherwise be removed. `Heatbox.destroy()` calls `_teardownTimeController()`. `Heatbox.updateOptions()` compares a serialised signature of the `temporal` options to decide whether to re-initialise, but every function is reduced to `[Function:name]`; replacing `dataSource` with a different function having the same name (including another anonymous function) can collide and leave the old controller in use.

Cesium の `Event.addEventListener` は解除関数を返すため、束縛済みハンドラを再導出するのではなく解除関数を保持している — `this._onTick.bind(this)` は呼び出しごとに新しい関数オブジェクトを生成するので、そうしなければ解除できない。`Heatbox.destroy()` は `_teardownTimeController()` を呼ぶ。`Heatbox.updateOptions()` は `temporal` オプションの直列化signatureで再初期化の要否を判断するが、すべての関数を `[Function:name]` に縮約する。異なる `dataSource` を同名関数（別のanonymous関数を含む）へ差し替えるとsignatureが衝突し、古いcontrollerが残る場合がある。
...
- `dataSource` removes the requirement to preload the full time range before the first frame. Loaded entries are retained without eviction, so this is deferred loading rather than bounded-memory streaming.
  `dataSource` により、初回frame前に全期間を事前ロードする必要がなくなる。ただしload済みentryはevictionなしで保持されるため、bounded-memory streamingではなく遅延ロードである。
...
- The concurrency guards cover two important cases: a completed slice resolution is discarded when a newer request id exists, and concurrent Heatbox updates coalesce to one queued entry. They do not make the whole asynchronous path race-free; the limitations below describe uncovered load, statistics, and teardown cases.
  並行制御は2つの重要な場合を扱う。より新しいrequest idがあれば完了したslice解決を破棄し、同時Heatbox更新は1件のqueued entryへ合流する。ただし非同期経路全体をrace-freeにはしない。未対応のload・統計・破棄caseは以下の制約に示す。
- Listener and current-worker cleanup is centralised through `deactivate()`, but teardown is not complete for already-started updates, pending data-source work, all pending-map failure modes, or every Blob URL.
  listenerと現在のworkerの解放は `deactivate()` に集約されるが、開始済み更新、pending中のdata-source処理、pending-mapの全failure mode、すべてのBlob URLまで完全には解放しない。
...
- **Lazy loads do not refresh the live global domain.** A merge clears `TimeSlicer`'s cache but does not update `Heatbox._globalStats`, which `TimeController` continues to pass as `_externalStats`. Initial partial-data statistics can therefore remain in force.
  **lazy loadは実描画のglobal domainを更新しない。** mergeは `TimeSlicer` のcacheを消すが、`TimeController` が `_externalStats` として渡し続ける `Heatbox._globalStats` は更新しない。そのため初期partial-data統計が残り得る。
- **Global single-flight can lose the newest time-specific miss.** A second miss for another time waits for the first pending load and, if that result does not cover the second time, returns `null` or interpolation without issuing a second load in the same call.
  **global single-flightは最新時刻固有のmissを取り逃し得る。** 別時刻の2番目のmissは最初のpending loadを待ち、その結果が2番目の時刻を含まなくても、同じ呼び出し内では2回目のloadを発行せず `null` または補間へ進む。
- **Global-stat initialisation commits before cancellation is checked.** An old controller can finish after deactivation and assign its result to `Heatbox._globalStats` before it notices that it is inactive, potentially overwriting a newer controller's domain.
  **global統計初期化はcancel確認より前にcommitする。** 旧controllerはdeactivate後に完了し、inactiveを確認する前に結果を `Heatbox._globalStats` へ代入できるため、新controllerのdomainを上書きし得る。
- **`hold` disables camera refresh while out of range.** The visual remains, but `_lastEntry` becomes `null`; `_onCameraChanged` then returns without re-planning the held voxel set for the new view.
  **範囲外の `hold` 中はcamera refreshが停止する。** 表示は残るが `_lastEntry` は `null` になり、`_onCameraChanged` は保持中voxel setを新しい視点で再計画せずreturnする。
- **Loaded entries accumulate.** `_mergeLoadedEntries` appends into `_entries` and has no eviction or time-window policy; memory use approaches full residency as the caller traverses the full dataset.
  **load済みentryは蓄積する。** `_mergeLoadedEntries` は `_entries` へ追加し、evictionやtime-window policyを持たない。全datasetを走査すればmemory useはfull residencyへ近づく。
- **Function-valued option changes can collide.** Temporal option signatures retain only a function's name, so same-named replacement functions can fail to trigger controller reconstruction.
  **関数値optionの変更は衝突し得る。** temporal option signatureは関数名だけを保持するため、同名の差し替え関数ではcontroller再構築が発火しない場合がある。
- **Worker failure cleanup has gaps.** A synchronous `postMessage` throw retains its pending-map entry, and construction/runtime failures can leave an older Blob URL unreclaimed when a later creation overwrites `_workerUrl`.
  **worker failure cleanupには不足がある。** 同期的な `postMessage` throwはpending-map entryを残し、生成／runtime failureでは後の生成が `_workerUrl` を上書きすると古いBlob URLが未回収になり得る。
    ...
    alt throttled
        ...
        alt sync path: no dataSource and no useWorker
            ...
            opt direct miss and dataSource configured
                TS->>TS: await one global pendingLoad, not keyed by requested time
                TS->>TS: merge entries, invalidate slicer stats cache only
            end
        ...
        alt entry is null
            ...
            HB->>VR: render voxelData, bounds, grid, statistics
            VR->>GR: beginFrame
            ...
            alt new voxel key
                ...
                GR->>VE: mutate Entity and BoxGraphics properties in place
            end
            VR->>GR: endFrame
...
**日本語での読み方**: 上から順に、`clock.onTick` → 実時間 throttle（既定 100 ms）→ 同期経路と非同期経路の分岐 → 非同期経路では request id による陳腐化判定 → 参照同一性による変更検知 → 進行中なら単一スロットへ待機（後着優先）→ `null` なら `outOfRangeBehavior` に従いクリア → データありなら `updateValues`（境界が許容差内なら既存グリッド再利用、そうでなければ `setData` で再構築）→ 統計再計算 → `VoxelRenderer` を介した差分描画（`beginFrame`/`plan`/`syncVoxel`/`endFrame`）。新規keyでは `viewer.entities` のmembershipを追加し、既存keyでは `Entity`／`BoxGraphics` の状態をその場で変更する。図の最下部は独立した経路であり、`camera.changed` が 50 ms throttle のうえ *同一スライス* を再投入して `RenderPlanner` に可視性を再評価させる。
...
**F. Interpolate positions as well as values.** Would allow moving points. Rejected for now: interpolating positions changes which voxel a point falls into, which invalidates grid reuse and can make voxel geometry appear to drift. Values-only interpolation keeps the spatial structure stable only when array ordering/identity is stable across slices; the current index-first pairing does not repair reordered same-length arrays by id.

**F. 値だけでなく位置も補間する。** 移動する点を扱える。現時点では却下: 位置を補間すると点が属するボクセルが変わり、グリッド再利用が無効化され、ボクセルジオメトリが漂動して見え得る。値のみの補間が空間構造を安定させるのはslice間で配列順序／identityが安定している場合に限られ、現状のindex-first pairingは同じ長さの配列の並べ替えをidで修復しない。
...
- `TemporalWorkerBridge` correlates requests by id, returns `null` when construction is unavailable, rejects worker errors, and revokes its Blob URL on destroy. `TimeSlicer`, not the bridge, performs the main-thread fallback after `null`, rejection, or an unusable result.
  `TemporalWorkerBridge` は id でリクエストを対応付け、生成不能時は `null` を返し、worker エラー時は reject し、破棄時に Blob URL を revoke する。`null`・reject・利用不能な結果の後にメインスレッド処理へフォールバックするのは bridge ではなく `TimeSlicer` である。
...
- **Interacts with ADR-0016 / ADR-0017** (classification): `classificationScope: 'global'` supplies a precomputed domain to `DataProcessor` through `_externalStats`, so classification breaks stay fixed across the entries present at initialisation. Lazy-loaded entries do not refresh the live domain, as documented above.
  **ADR-0016 / ADR-0017（分類）と相互作用する。** `classificationScope: 'global'` は `_externalStats` を通じて事前計算済みドメインを `DataProcessor` に渡すため、初期化時に存在するentry間では分類breakが固定される。上記のとおり、lazy-loaded entryは実描画domainを更新しない。
