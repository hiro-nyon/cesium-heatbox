...
**Implementation State**: Implemented / Evolved — extended by ADR-0021. See *Implementation Outcome* at the end of this document.
**Related**: ROADMAP v1.2.0, ADR-0017 (Classification Extension), ADR-0021 (Asynchronous Temporal Pipeline)

> **Status vocabulary note (2026-08-23)**: this ADR originally recorded its status as `Approved`. The value has been
> normalised to `Accepted` to match the project's decision-status vocabulary
> (`Proposed` / `Accepted` / `Rejected` / `Deprecated` / `Superseded`). The decision itself is unchanged.

> **Historical Note on performance figures (2026-08-23)**: this document states performance targets — including
> "10,000点のデータで 60fps 以上を維持", global-scope initialisation under 500 ms, and update times under
> 50 ms / 100 ms. These are **historical design targets set at the time of this ADR; they are not current
> cross-device performance guarantees.** No real-browser frame-rate benchmark exists in this repository, and the
> wall-clock performance tests under `test/performance/` run against a mocked Cesium with several files excluded
> from the default Jest suite. The figures are retained verbatim as part of the historical record and must not be
> quoted as current guaranteed performance. See ADR-0019 *Known Limitations*.
...
> Everything above this line is the historical record of the v1.2.0 proof-of-concept decision and has not been rewritten.

### The PoC architecture is still in force

`TimeController` and `TimeSlicer` exist at the intended paths with the intended split of responsibilities. The `temporal` option group, the `global` / `per-time` classification scopes, the `hold` / `clear` out-of-range behaviour, the `skip` / `prefer-earlier` / `prefer-later` overlap policy, and the `updateInterval` throttle are all part of the current public surface, normalised in `src/utils/validation.js`. Clock integration is still `viewer.clock.onTick`, and slice lookup still uses the cached-entry → neighbour-scan → binary-search sequence designed here.

### Deferred items that were subsequently implemented

Four capabilities this ADR placed out of scope or rejected as alternatives were built in the v1.3.x line. The exact delivery points changed: interpolation and lazy loading moved from the planned v1.3.0 to v1.3.2, and the lightweight update API moved from the planned v1.3.0 to v1.3.1:

| This ADR said | Outcome |
|---|---|
| **Out of Scope**: データの補間（Interpolation）: v1.3.0 | Implemented as `temporal.interpolate` in **v1.3.2** (`86ef7f0`). Numeric fields are linearly interpolated across gaps; `position` is deliberately *not* interpolated, so voxel geometry does not drift between slices. |
| **Out of Scope**: 遅延ロード（Lazy Loading）: v1.3.0 | Implemented as `temporal.dataSource` in **v1.3.2** (`86ef7f0`), an async provider with a single-flight guard. Lazily merged entries are re-normalised through the same overlap resolution as eager data, and merging invalidates the global statistics cache. |
| **Alternative 2 (WebWorker)** — rejected for v1.2.0, "v1.3.0 以降でオプション機能として検討。`temporal.useWorker: true` のような設定で有効化" | Implemented as `temporal.useWorker` in **v1.3.2** (`86ef7f0`), under that exact option name. The worker handles numeric preprocessing only and falls back to the main thread whenever it is unavailable, errors, or returns an unusable result. |
| **Alternative 4 (setData 軽量化)** — rejected as a public API, "v1.3.0 で `heatbox.updateValues(data)` として検討" | Implemented as `Heatbox.updateValues(entities, runtimeOptions?)` in **v1.3.1** (`eabfd9e`), under that exact method name. `TimeController` now prefers it over `setData()`. |

The concurrency machinery that accompanies those additions — request-id staleness rejection, single-slot coalescing of in-flight updates, and camera-triggered refresh — did not exist in v1.2.0 and was introduced because asynchronous slice resolution made it necessary.

**Alternative 3 (3D Tiles integration)** was deferred to v2.0 and remains **unimplemented**. `Cesium3DTileset` appears nowhere in `src/`.

### One item that became vestigial

This ADR introduced `options._skipRebuild` as an internal flag. `TimeController` still passes `_skipRebuild: false` in its update options, but **nothing in `src/` reads that flag**. Its role was taken over by the `updateValues()` / `setData()` decision in ADR-0021, which centralises the grid-reuse policy in `Heatbox._canReuseGridForUpdate`.

### Further reading

- **ADR-0021: Asynchronous Temporal Data Pipeline and Lightweight Updates** — extends this ADR with interpolation, lazy loading, worker preprocessing, the `updateValues()` fast path, and the concurrency guards. It does not supersede this document.
- **ADR-0019: Current Runtime Architecture and CesiumJS Integration** — the current architecture map, including the temporal subsystem's place in it and its verified limitations.
