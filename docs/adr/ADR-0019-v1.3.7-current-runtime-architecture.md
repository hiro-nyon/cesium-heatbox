...
**Date**: 2026-08-23
**Author**: hiro-nyon
**Describes / 記述対象**: v1.3.7 — source pinned at commit [`32bdfcd8ce4e9e983f42deea222a9e1630e7ce5c`](https://github.com/hiro-nyon/cesium-heatbox/tree/32bdfcd8ce4e9e983f42deea222a9e1630e7ce5c)
**Related**: [ADR-0009](ADR-0009-voxel-renderer-responsibility-separation.md), [ADR-0013](ADR-0013-v0.1.17-spatial-id-support.md), [ADR-0016](ADR-0016-v1.0.0-classification-engine.md), [ADR-0017](ADR-0017-v1.1.0-classification-extension.md), [ADR-0018](ADR-0018-v1.2.0-time-dependent-data.md), [ADR-0020](ADR-0020-v1.3.0-incremental-entity-rendering.md), [ADR-0021](ADR-0021-v1.3.2-asynchronous-temporal-pipeline.md)
...
>
> **Audience.** An experienced CesiumJS or geospatial software engineer who has never seen this repository. Written bilingually: English first, Japanese immediately after. ADR-0001 through ADR-0018 are predominantly Japanese (ADR-0013 onward are partly bilingual) and are retained as historical records.
>
> **想定読者.** 本リポジトリを見たことがない、経験のある CesiumJS／地理空間ソフトウェアエンジニア。日英併記（英語の直後に日本語）。ADR-0001〜ADR-0018 は主に日本語（ADR-0013以降は一部日英併記）であり、歴史的記録として保持している。
>
> **Verification basis.** Every module, class, method, and Cesium API named here was read in `src/` at the commit above. Diagram B shows concrete dependencies; Diagrams A and C intentionally mix calls with data/control flow and are labelled accordingly. Claims about performance and about capabilities the library does **not** have are stated explicitly in *Known Limitations*.
>
> **検証の基準.** ここで名前を挙げたモジュール、クラス、メソッド、Cesium API はすべて上記コミット時点の `src/` で確認した。図Bは具体的な依存を示し、図A/Cはcallとdata/control flowを意図的に混在させ、その旨を各図に明記する。性能に関する主張と、ライブラリが **持たない** 機能については *Known Limitations* に明記する。
>
...
5. **Responsibility separation.** A 1,265-line `VoxelRenderer` was decomposed into colour, selection, adaptive-control, and geometry components (ADR-0007 → ADR-0008 → ADR-0009). ADR-0007 was superseded before implementation; ADR-0008 was implemented in phases, then reverted in `299d522` before ADR-0009 replaced it.
   **責務分離.** 1,265行の `VoxelRenderer` を色・選択・適応制御・ジオメトリの各コンポーネントへ分解（ADR-0007 → ADR-0008 → ADR-0009）。ADR-0007は実装前に置き換えられた。ADR-0008は段階的に実装された後、`299d522` でrevertされ、ADR-0009に置き換えられた。
...
6. **Maintain predictable lifecycle cleanup for tracked resources.** `destroy()` releases the persistent click handler, performance-overlay listener, temporal listeners and worker, UI objects, and renderer-owned entities. A one-shot `fitView()` post-render listener is locally scoped and is not tracked by `destroy()`, and an `updateValues()` call that already started is not cancelled.
   **追跡対象リソースの予測可能なライフサイクル解放を維持する。** `destroy()` は永続click handler、performance overlay listener、temporal listenerとworker、UI object、renderer所有entityを解放する。一方、`fitView()` のone-shot post-render listenerはローカルスコープで `destroy()` の追跡対象ではなく、既に開始済みの `updateValues()` もキャンセルされない。
7. **Remain reusable as a client-side library.** Published to npm with ESM, CommonJS, and UMD builds, conditional TypeScript declarations, a declared Cesium peer dependency range, and a CI dependency/build matrix for both the minimum and latest Cesium. That matrix does not execute real Cesium runtime APIs, as qualified below.
   **再利用可能なクライアントサイドライブラリであり続ける。** ESM／CommonJS／UMD ビルド、条件付き TypeScript 宣言、Cesium peer dependency 範囲の明示、最小版と最新版に対する CI dependency/build matrixを伴ってnpm公開されている。ただし後述のとおり、このmatrixは実Cesium runtime APIを実行しない。
...
**`CoordinateTransformer`** (`src/core/CoordinateTransformer.js`) — computes the 3D geographic bounds of an entity array. For each entity it resolves `entity.position`, calling `getValue(JulianDate.now())` when the position is a Cesium `Property` and reading it directly otherwise, then converts `Cartesian3` to `Cartographic` via `Cesium.Cartographic.fromCartesian` and to degrees via `Cesium.Math.toDegrees`. Missing positions and null conversion results are skipped silently; thrown conversion errors are skipped with a warning.

**`CoordinateTransformer`** — エンティティ配列の3D地理境界を計算する。各エンティティについて `entity.position` を解決し（Cesium `Property` なら `getValue(JulianDate.now())`、そうでなければ直接読み取り）、`Cesium.Cartographic.fromCartesian` で `Cartesian3` を `Cartographic` へ、`Cesium.Math.toDegrees` で度へ変換する。position欠落と変換結果nullは警告なしでスキップし、変換が例外を投げた場合は警告つきでスキップする。
...
**`DataProcessor`** (`src/core/DataProcessor.js`) — assigns entities to voxels and computes statistics. `classifyEntitiesIntoVoxels` branches on `options.spatialId.enabled` between the regular-grid path and the Spatial ID path. Both paths resolve raw positions and convert Cartesian coordinates with similar duck-typed logic, then produce a `Map` of compact voxel records — `{ x, y, z, count }` plus optional `layerStats`, `layerTop`, `spatialId`, and `bounds`. Only the regular-grid path performs the explicit bounds-tolerance rejection (`0.001` degrees horizontally and `1` metre vertically); the Spatial ID path passes resolved coordinates directly to the adapter. Source entities are **not** retained on the records. When aggregation is enabled it resolves a layer key per entity, reading a `Cesium.PropertyBag` via `entity.properties.getValue(currentTime)` with a direct-property fallback, and computes the dominant layer per voxel. `calculateStatistics` produces the density statistics and classification metadata, and accepts a precomputed domain through `options._externalStats` for temporal global scope.

**`DataProcessor`** — エンティティをボクセルへ割り当て、統計を計算する。`classifyEntitiesIntoVoxels` は `options.spatialId.enabled` で一様グリッド経路と空間ID経路に分岐する。両経路は類似したduck-typed logicで生positionの解決とCartesian座標変換を行い、コンパクトな voxel record の `Map` を生成する（`{ x, y, z, count }` に任意の `layerStats`、`layerTop`、`spatialId`、`bounds`）。明示的なbounds許容差判定（水平 `0.001` 度、垂直 `1` m）を行うのは一様グリッド経路だけであり、空間ID経路は解決した座標を直接adapterへ渡す。元エンティティは record に **保持しない**。集約有効時はエンティティごとにレイヤキーを解決する（`entity.properties.getValue(currentTime)` で `Cesium.PropertyBag` を読み、直接プロパティへフォールバック）。`calculateStatistics` は密度統計と分類メタデータを生成し、時系列 global scope 用に `options._externalStats` 経由の事前計算済みドメインを受け付ける。
...
**Attribution.** The ZFXY encoding implementation used in the primary path is third-party: [`ouranos-gex-lib-for-javascript`](https://github.com/ODS-IS-STID/ouranos-gex-lib-for-JavaScript), MIT-licensed, with its bundled notice identifying `Copyright (c) 2022 Geolonia Inc.`, and pinned in this repository to commit `5742889f20645fb0451c4870e5c54b5d34ab5c31`. The source repository is currently hosted under the `ODS-IS-STID` organisation. This project authored the adapter, the zoom-selection logic, the tile traversal helpers, the fallback converter, and the integration into voxel assignment — not the provider's Spatial ID implementation.

**帰属.** 主経路で用いる ZFXY エンコーディング実装は第三者製である — `ouranos-gex-lib-for-javascript`、MITライセンス、同梱noticeは `Copyright (c) 2022 Geolonia Inc.` と記載し、本リポジトリではコミット `5742889f20645fb0451c4870e5c54b5d34ab5c31` に固定している。source repositoryは現在 `ODS-IS-STID` organization配下で公開されている。本プロジェクトが著したのはアダプタ、ズーム選択ロジック、タイル走査ヘルパー、フォールバックコンバータ、およびボクセル割り当てへの統合であり、provider の空間ID実装そのものではない。
...
**`VoxelSelector`** (`src/core/selection/VoxelSelector.js`) — selection strategies `density`, `coverage` (stratified), and `hybrid`, with TopN forcing and a density-sort fallback on error. Cesium-free and directly unit-testable, but stateful: it retains options and last-selection statistics and emits logs.

**`VoxelSelector`** — 選択戦略 `density`、`coverage`（層化抽出）、`hybrid`。TopN の強制包含に対応し、エラー時は密度ソートへフォールバックする。Cesium非依存で直接unit test可能だが、optionsと直前selection統計を保持しlogも出すstateful componentである。
...
**`RenderPlanner`** (`src/core/render/RenderPlanner.js`) — reads camera state and returns a plan: `plan(voxels, bounds, grid, topNVoxels, baseBudget) -> { voxels, budget, culledCount }`. Applies an angular visibility filter, a height-scaled budget, and a weighted additive priority score combining TopN membership, density, and camera proximity. The weights do not guarantee a strict lexicographic ordering between those factors. Reads the camera; never mutates entities. See ADR-0020.

**`RenderPlanner`** — カメラ状態を読んで計画を返す。角度による可視性フィルタ、高度スケール予算、そして TopN 所属・密度・カメラ近接を合算する重み付き優先度スコアを適用する。この重みは各要因間の厳密な辞書式順序を保証しない。カメラを読むのみで、エンティティを変更しない。ADR-0020 参照。
...
**`TemporalWorkerBridge`** / **`TemporalWorkerScript`** / **`temporalWorkerTasks`** (`src/core/temporal/`) — an optional Web Worker whose production path currently performs interpolation only. The protocol also defines a `stats` branch, but the generated script omits a helper required by that branch, so a real stats task currently errors; no production code calls it. The worker script is generated by serialising pure functions in `temporalWorkerTasks.js` with `Function.prototype.toString()` into a Blob, so it contains no Cesium code and no DOM access. The bridge correlates requests by id and returns `null` or rejects on failure; `TimeSlicer` owns the main-thread fallback. See ADR-0021.

**`TemporalWorkerBridge`** / **`TemporalWorkerScript`** / **`temporalWorkerTasks`** — production経路では現在補間のみを行う任意の Web Worker。protocolには `stats` 分岐もあるが、生成scriptが必要helperを含めないため実際のstats taskは現在エラーになり、productionからの呼び出しも無い。worker script は `temporalWorkerTasks.js` の純粋関数を `Function.prototype.toString()` で Blob へ直列化して生成されるため、Cesium コードも DOM アクセスも含まない。bridge は id でリクエストを対応付け、失敗時は `null` またはrejectを返す。メインスレッドへのfallbackを所有するのは `TimeSlicer` である。ADR-0021 参照。
...
`Heatbox` → `VoxelRenderer` → { `VoxelSelector`, `AdaptiveController`, `RenderPlanner`, `GeometryRenderer`, `ColorCalculator` }, and `Heatbox` → { `CoordinateTransformer`, `VoxelGrid`, `DataProcessor`, `TimeController` }; `DataProcessor` → `SpatialIdAdapter` → `ZFXYConverter`; `TimeController` → { `Heatbox` instance, `TimeSlicer` }; `TimeSlicer` → { `CoordinateTransformer`, `VoxelGrid`, `DataProcessor`, `TemporalWorkerBridge` }. The runtime object graph therefore contains an intentional `Heatbox` ↔ `TimeController` cycle: `Heatbox` constructs the controller, and the controller retains the facade it updates. There are no reverse references among the sibling render components.

runtime object graphには意図的な `Heatbox` ↔ `TimeController` の循環がある。`Heatbox` がcontrollerを生成し、controllerは更新対象facadeを保持する。一方、兄弟関係にある描画コンポーネント同士の逆参照は存在しない。
...
| `JulianDate` | Simulation-time parsing, comparison, `secondsDifference`, `addSeconds`, and `now`; wall-clock throttling separately uses `Date.now()`.<br>simulation-timeのparse、比較、`secondsDifference`、`addSeconds`、`now`。wall-clock throttleは別途 `Date.now()` を使用 |
...
| Teardown of tracked persistent resources; known gaps for one-shot `fitView` listeners and already-started updates<br>追跡済み永続リソースの破棄（one-shot `fitView` listenerと開始済み更新には既知の不足あり） | `Heatbox.destroy` |
...
Conceptual data and control flow for the static path, plus the camera's read-only input to planning. Edges intentionally mix calls, returned data, and processing stages; Diagram B carries the concrete dependency graph.

静的経路の概念的なdata/control flowと、計画へのcameraからのread-only入力。辺にはcall、return data、処理段階が意図的に混在する。具体的な依存graphは図Bに示す。
...
flowchart LR
    ...
    subgraph WORKER["Web Worker: optional, Cesium-free by construction"]
        WK["Blob worker: interpolation works; stats branch currently misses a helper"]
    end
...
Condensed. ADR-0021 carries the expanded intended sequence and the principal fallback branches, with uncovered races documented in its trade-offs.

要約版である。想定sequenceと主要fallback branchの拡張版はADR-0021にあり、未対応raceは同ADRのtrade-offに記載する。
...
sequenceDiagram
    ...
    participant VE as existing voxel Entity state
    ...
    alt sync path: no dataSource and no useWorker
        ...
        opt lazy load needed
            TS->>TS: global single-flight load, merge, invalidate slicer cache only
        end
    ...
    alt slice is null
        ...
        HB->>VR: render current voxel state
        VR->>GR: beginFrame
        VR->>RP: plan with camera state
        RP-->>VR: prioritized voxels within budget
        VR->>GR: syncVoxel per planned key
        alt new voxel key
            GR->>EC: add entity membership
        else existing voxel key
            GR->>VE: mutate Entity and BoxGraphics properties
        end
        VR->>GR: endFrame
...
**日本語での読み方**: `clock.onTick` から始まり、実時間 throttle（既定 100 ms）を通過した後、`dataSource` または `useWorker` の設定有無で同期経路と非同期経路に分岐する。非同期経路では request id による陳腐化判定が入る。続いて参照同一性による変更検知、進行中なら単一スロットへの待機（後着優先）、`null` なら `outOfRangeBehavior` に従うクリア、データありなら `updateValues`（境界が許容差内なら既存グリッド再利用、そうでなければ `setData` で完全再構築）。最後に `VoxelRenderer` を介して差分描画する。新規keyはEntityCollection membershipを追加し、既存keyはEntity/BoxGraphicsの状態をその場で変更する。図の末尾は独立した経路で、`camera.changed` が50 ms throttleのうえ同一スライスを再投入し、カメラ考慮フィルタを再評価させる。

## CesiumJS version compatibility policy / CesiumJS バージョン互換性ポリシー

The supported range is declared, and CI verifies dependency installation and buildability at both ends. Runtime API compatibility is not enforced by that matrix:

対応範囲は宣言され、CIが両端で依存導入とbuildabilityを検査する。ただしruntime API互換性を強制するmatrixではない。
...
- CI installs `['1.120.0', 'latest']` and runs selected Jest tests plus a build. The minimum dependency snapshot is pinned (`cesium@1.120.0`, `@cesium/engine@10.1.0`, `@cesium/widgets@7.1.0`), while the upper target follows the current release. However, Jest maps every `cesium` import to `test/__mocks__/cesium.js`, webpack treats Cesium as external, and the package smoke test checks exports/types rather than invoking Cesium. The matrix therefore proves install/build compatibility, not real Cesium runtime compatibility.
  CIは `['1.120.0', 'latest']` を導入して選択Jest testとbuildを実行する。最小dependency snapshotは固定され、上限targetは現行releaseに追随する。ただしJestは全 `cesium` importを `test/__mocks__/cesium.js` へmapし、webpackはCesiumをexternal扱いし、package smoke testはCesium API呼び出しではなくexports/typesを検査する。したがってmatrixが証明するのはinstall/build互換性であり、実Cesium runtime互換性ではない。
...
- **Centralised cleanup for tracked resources.** One `destroy()` releases the persistent click handler and overlay listener, the overlay, the legend, the temporal controller with its clock and camera listeners, its current worker, and renderer-owned entities. Known gaps are listed below rather than treated as covered by this guarantee.
  **追跡済みリソースの集中解放。** 1回の `destroy()` で、永続click handlerとoverlay listener、overlay、凡例、clock/カメラリスナを伴うtemporal controller、その時点のworker、renderer所有entityを解放する。この保証に含まれない既知の不足は後述する。
...
4. **Camera-aware filtering is heuristic, not culling.** A single circular cone test against `camera.direction` uses an ad-hoc allowance derived from `frustum.fov` and aspect ratio and tests only each voxel's estimated centre. Cesium defines `fov` as horizontal when width exceeds height and vertical otherwise, but the implementation always applies aspect-ratio expansion, so the allowance is not a true horizontal half-FOV and can produce both false positives and false negatives against the rectangular frustum. Voxel centres are ECEF while the camera side reads `position`/`direction`, not `positionWC`/`directionWC`, so non-identity camera transforms are not verified. There are no frustum planes, near/far tests, separate vertical bound, terrain occlusion, or GPU occlusion. Budget scaling is a four-step ratio on normalised camera height, not a level-of-detail system.
   **カメラ考慮のフィルタはヒューリスティックであり culling ではない。** `frustum.fov` とaspect ratioから導くad-hocな許容角を用い、各voxelの推定中心だけを `camera.direction` に対する単一の円形円錐で判定する。Cesiumの `fov` はwidth > heightなら水平、そうでなければ垂直だが、実装は常にaspect-ratio拡張を適用するため、真の水平半FOVではなく矩形frustumに対してfalse positive/negativeの両方を生じ得る。voxel中心はECEFだがcamera側は `positionWC`／`directionWC` ではなく `position`／`direction` を読むため、non-identity camera transformは未検証である。視錐台平面、near/far判定、独立した垂直境界、地形／GPU遮蔽はいずれも無い。予算スケーリングは正規化カメラ高度に基づく4段階比率であり、LoDシステムではない。
...
8. **The temporal worker preprocesses only, and currently only interpolation works.** All Cesium entity creation, mutation, and removal happens on the main thread. The protocol's `stats` branch omits `calculateTemporalValueStats` from the generated script and therefore errors. It is reachable only through `TimeSlicer.calculateGlobalStatsAsync()`, which no code in `src/` calls; the live global-scope path computes statistics on the main thread.
   **時系列 worker は前処理のみであり、現在機能するのは補間だけである。** Cesium エンティティの生成・変更・削除はすべてメインスレッドで行われる。protocolの `stats` 分岐は生成scriptに `calculateTemporalValueStats` を含めないためエラーになる。また到達できるのは `TimeSlicer.calculateGlobalStatsAsync()` 経由のみであり、`src/` のどのコードもこれを呼ばない。実際の global scope 経路はメインスレッドで統計を計算する。
...
13. **No real-Cesium browser/runtime test matrix.** CI installs minimum/latest Cesium, but Jest substitutes the repository mock and the build externalises Cesium. Transpile targets and a manual compatibility checker exist; there is no automated real-Cesium browser smoke test or WebGL visual-regression suite.
    **実Cesium browser/runtimeのテストマトリクスは無い。** CIは最小／最新Cesiumを導入するが、Jestはrepository mockへ置換し、buildはCesiumをexternal化する。transpile targetと手動互換性checkerは存在するが、自動の実Cesium browser smoke testやWebGL視覚回帰suiteは無い。
...
15. **Teardown has asynchronous gaps.** `destroy()` does not track a pending one-shot `fitView()` post-render listener and does not cancel an already-started `updateValues()`. A pending `dataSource` promise is also not cancelled or invalidated by `TimeSlicer.destroy()`; after it resolves, a later interpolation attempt can cause the destroyed bridge to create another worker because the bridge has no destroyed guard. Worker construction/runtime failures can additionally overwrite an unreleased Blob URL before final teardown.
    **破棄処理には非同期上の不足がある。** `destroy()` は pending中のone-shot `fitView()` post-render listenerを追跡せず、開始済みの `updateValues()` もキャンセルしない。pending中の `dataSource` promiseも `TimeSlicer.destroy()` で取消・無効化されず、解決後の補間処理がdestroyed guardを持たないbridgeにworkerを再生成させ得る。worker生成／runtime failureでは、未revokeのBlob URLが最終破棄前に上書きされる場合もある。
16. **Temporal lazy-loading and configuration have correctness limits.** The single-flight guard is global rather than keyed by requested time, so a later miss can wait for an unrelated load and return without issuing its own request. Lazy merges invalidate only the slicer's statistics cache, not the live `Heatbox._globalStats` domain. Loaded entries accumulate without eviction. An old global-stat initialisation can assign after deactivation, same-named function replacements can collide in the temporal option signature, and `outOfRangeBehavior: 'hold'` leaves a visual that camera changes do not re-plan because `_lastEntry` is `null`.
    **temporal lazy-loadingと設定には正しさ上の制約がある。** single-flight guardはrequested timeでkeyed化されないglobal 1本であり、後発missが無関係なloadを待って自分用requestを発行せずreturnし得る。lazy mergeが無効化するのはslicerの統計cacheだけで、実描画の `Heatbox._globalStats` domainではない。load済みentryはevictionなしで蓄積する。旧global統計初期化はdeactivate後に代入でき、同名関数の差し替えはtemporal option signatureで衝突し、`outOfRangeBehavior: 'hold'` では `_lastEntry` が `null` のため保持表示をcamera変更時に再計画しない。
17. **Runtime selection options are not fully propagated.** `Heatbox.updateOptions()` refreshes the renderer, adaptive controller, geometry renderer, and render planner, but not the already-constructed `VoxelSelector`. Changes to selector-owned options such as `renderLimitStrategy`, `coverageBinsXY`, and `minCoverageRatio` therefore do not affect that instance until reconstruction.
    **runtime selection optionは完全には伝播しない。** `Heatbox.updateOptions()` はrenderer、adaptive controller、geometry renderer、render plannerを更新するが、生成済み `VoxelSelector` は更新しない。そのため `renderLimitStrategy`、`coverageBinsXY`、`minCoverageRatio` などselector所有optionの変更はinstance再生成まで反映されない。

## Relationship to Previous ADRs / 過去ADRとの関係

| ADR | Status of its decision today / 現在におけるその決定の状態 |
|---|---|
| **[ADR-0009](ADR-0009-voxel-renderer-responsibility-separation.md)** — VoxelRenderer responsibility separation | **In force, extended.** `ColorCalculator`, `VoxelSelector`, `AdaptiveController`, and `GeometryRenderer` all exist with the intended responsibilities, and the dependency rules hold. Two divergences: the 300-line target for `VoxelRenderer` was not met (802 lines), and `GeometryRenderer`'s contract became the frame protocol instead of `renderVoxel(...) => Entity[]`. `RenderPlanner` is a Cesium-reading component the original taxonomy did not anticipate. See ADR-0009's Implementation Outcome.<br>**有効かつ拡張済み。** 4クラスは意図された責務で存在し、依存規則も守られている。乖離2点 — `VoxelRenderer` の300行目標は未達（802行）、`GeometryRenderer` の契約は `renderVoxel(...) => Entity[]` からフレームプロトコルへ変化。`RenderPlanner` は当初の分類が想定しなかった Cesium 読み取りコンポーネントである |
| **[ADR-0013](ADR-0013-v0.1.17-spatial-id-support.md)** — Spatial ID support | **In force, evolved.** `SpatialIdAdapter` and `ZFXYConverter` work as designed. The dependency model changed in v1.3.7: the third-party provider is now a commit-pinned build dependency bundled as a lazy chunk, so consumers need no extra install. See ADR-0013's Implementation Outcome.<br>**有効かつ発展済み。** 両クラスは設計どおり動作する。依存モデルは v1.3.7 で変化し、第三者 provider はコミット固定のビルド依存として遅延チャンクに同梱されるため利用者の追加インストールは不要 |
| **[ADR-0016](ADR-0016-v1.0.0-classification-engine.md) / [ADR-0017](ADR-0017-v1.1.0-classification-extension.md)** — classification engine and extension | **In force.** All seven schemes are implemented, `classificationTargets` covers colour, opacity, and width, and `simple-statistics` backs quantile and Jenks. Precomputed breaks let temporal global scope fix a domain across the entries present at initialisation; lazy merges do not refresh that live domain.<br>**有効。** 7スキームすべて実装済み、`classificationTargets` は色・透明度・線幅をカバー、quantile と Jenks は `simple-statistics` が支える。事前計算済みbreaksによりtemporal global scopeは初期化時entry間でdomainを固定するが、lazy mergeは実描画domainを更新しない |
| **[ADR-0018](ADR-0018-v1.2.0-time-dependent-data.md)** — temporal PoC | **In force, extended by ADR-0021.** The `temporal` option surface, the `TimeController`/`TimeSlicer` split, classification scopes, and overlap policy are all still current. Its performance figures are historical design targets.<br>**有効、ADR-0021 が拡張。** `temporal` オプション面、`TimeController`/`TimeSlicer` の分割、分類スコープ、重複解消ポリシーはいずれも現行。性能数値は historical design target である |
| **[ADR-0020](ADR-0020-v1.3.0-incremental-entity-rendering.md)** — incremental Entity rendering and camera-aware planning | **Current.** Defines today's render lifecycle and the camera-aware planning stage described above.<br>**現行。** 上記で述べた現在の描画ライフサイクルとカメラ考慮の計画段階を定義する |
| **[ADR-0021](ADR-0021-v1.3.2-asynchronous-temporal-pipeline.md)** — asynchronous temporal pipeline and lightweight updates | **Current.** Defines today's temporal execution paths, the `updateValues`/`setData` decision, and the worker boundary, including the documented limitations above.<br>**現行。** 現在の時系列実行経路、`updateValues`/`setData` の判断、worker 境界、および上記の既知制約を定義する |
...
| ADR-0007, ADR-0008 | ADR-0007 was superseded before implementation. ADR-0008 was implemented in phases, then reverted and superseded by ADR-0009. Both are retained to show the path taken.<br>ADR-0007は実装前に置換。ADR-0008は段階実装後にrevertされADR-0009へ置換。辿った経路を示すため両方を保持 |
