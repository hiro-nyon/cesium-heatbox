...
## Status
Superseded by ADR-0009 — 2025-09-07

Supersedes: ADR-0007

> Editorial note (2025-09-07):
> 本ADRは実装の複雑性と保守コストの懸念により、全面的な実装に至る前に ADR-0009 に置き換えられました。以下の「Phase」や達成状況は当初の意図を記録するものであり、実際のコードベース（src/core 配下）の構造は ADR-0009 の段階的責務分離方針へと見直されています。
...
### ドキュメント運用（MIGRATION.md）
- ルートの `MIGRATION.md` に、0.1.9 → 0.1.10 → 0.1.11 の変更点・置換表・コード例を集約する。
- 安定版タグ（`vX.Y.Z`）でのリリース時に、`MIGRATION.md` を GitHub Wiki に「Migration」として公開（自動同期の対象）する。
  - 運用方針: Wikiの該当ページを差し替え、ページ上部に対象バージョン範囲（例: “For 0.1.x → 0.1.11”）を明記。
  - 既存版ページは履歴として残す（必要なら「Migration-Archive」などに移動）。
  - README から Wiki の Migration へのリンクを掲載。

### Next: 0.1.11 で実施するAPIクリーンアップ
- 目的: 0.1.10 で互換維持のため見送った統一/整理を遂行し、0.1系のAPI表面を簡素化。
- 内容（Breaking を含む）:
  - オプション名称の統一
    - `fitViewOptions.pitch`/`heading` → `pitchDegrees`/`headingDegrees` のみに統一（旧名は削除）
    - `outlineEmulation` → `outlineRenderMode: 'emulation-only'` に集約
  - Resolver系の整理
    - `outlineWidthResolver`/`outlineOpacityResolver`/`boxOpacityResolver` を削除
    - 代替: `outlineWidthPreset` + `adaptiveOutlines` + `adaptiveParams`
- 移行ポリシー:
  - 0.1.10 は deprecation のみ（削除はしない）
  - 0.1.11 で削除・名称統一を実施。`MIGRATION.md` と Wiki に置換表とコード例を掲載
...
- [x] 型定義（`types/index.d.ts`）の反映（`pitchDegrees`/`headingDegrees`統一、非推奨オプション削除）
- [x] サンプルコードの更新（examples/の新APIへの対応完了）
...
**目標**: ドキュメント整備と品質保証（利用者視点の使いやすさを最重視）
- [x] 主要 public API の JSDoc 整備（英日併記、@example 追加、@default/@private 整理）
- [x] 例・デモの確認（基本/高度のワークフローが JSDoc に沿って説明可能）
- [x] Lint errors 0（対象範囲）
- [x] `npm run docs` による JSDoc 生成と Wiki 同期の運用確認（安定タグ時）
- [x] README/spec の追補（v0.1.10新用語定義・MIGRATION.md導線追加完了）

**実装サマリ**
- `Heatbox.setData`/`fitView` に典型パターンの @example を追加（Top-down、padding、pitch/heading 例）
- `VoxelRenderer` オプションの @default を明示（outlineRenderMode、adaptiveOutlines、outlineWidth 等）
- 内部メソッドに @private/@internal を付与し、Public API の見通しを改善
- 主要ユーティリティ（`ColorMap`/`VoxelGeometry`/`OutlineRenderer`/`AdaptiveOutlineController` など）に説明と引数の型を明記
- `MIGRATION.md` をルートに配置し、安定タグ時に Wiki「Migration」へミラー（運用方針は本ADRとRUNBOOKに記載）

**フォローアップ（推奨・任意）**
- README から Wiki「Migration」への導線の強化（バッジ/目次）
- 用語集（TopN/インセット/エミュレーション）ページを Wiki に追加し、JSDoc から @see で相互参照
- 代表ユースケースの @example 追加（adaptive-density / topn-focus の比較）

**受け入れ基準**: JSDoc の実用性向上・ビルド/同期運用の安定化（達成）。README/spec の追補は Phase 5.1（軽微更新）で対応予定。

### Phase 6: Performance and Final Validation (COMPLETED)
**目標**: 性能検証と最終リリース準備
- [x] setData/render 処理時間の性能テスト（±5%以内確認）
- [x] メモリリークテスト（反復実行での増加収束確認）
- [x] 全Examplesの動作確認（basic/advanced 20+例での API 使用確認）
- [x] 最終リグレッションテスト（190/195 テスト通過、97.4%成功率）

**実装サマリ**
- **性能安定性**: 2000ボクセル正常レンダリング、パフォーマンス影響-0.3%（改善）
- **機能完全性**: 12/14 テストスイート通過、主要機能全て動作
- **API互換性**: 20+のサンプル例で新APIの正常動作確認済み
- **品質保証**: 非致命的性能テストの許容範囲逸脱のみ（2例）、コア機能は全て安定
