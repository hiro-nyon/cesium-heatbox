...
**Status**: Accepted  
**Implementation State**: Implemented — `computeSpatialIdEdgeCaseMetrics` (`src/core/spatial/SpatialIdQaMetrics.js`)
runs once per Spatial ID render and is folded into `getStatistics()`; dateline wrapping is handled in
`SpatialIdAdapter.neighbors`. Dataset-level antimeridian crossing remains unsupported — see ADR-0019.  
**Date**: 2025-11-18  
**Author**: hiro-nyon  
...
> shipped in v0.1.19. The status has been corrected to `Accepted`; the decision text is unchanged.
