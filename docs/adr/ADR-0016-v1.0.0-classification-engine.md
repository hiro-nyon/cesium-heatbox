...
**Status**: Accepted  
**Implementation State**: Implemented — `linear`, `log`, `equal-interval`, `quantize`, and `threshold` are all
present in `src/utils/classification.js`, with `simple-statistics` behind `src/utils/classificationBackend.js`.  
**Date**: 2025-11-18  
**Author**: hiro-nyon  
...
> shipped in v1.0.0. The status has been corrected to `Accepted`; the decision text is unchanged. The
