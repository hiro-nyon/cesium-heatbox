...
**Implementation State**: Implemented / Evolved — see *Implementation Outcome* at the end of this document.  
**Related**: ROADMAP v0.1.17, PREP-ADR-0013, ADR-0015 (Global Spatial ID QA), ADR-0019 (Current Runtime Architecture)

> **Status note (2026-08-23)**: this ADR remained `Proposed` after the decision had been implemented and
> shipped in v0.1.17. The status has been corrected to `Accepted`; the decision text is unchanged.
...
> Added 2026-08-23. Verified against `src/`, `package.json`, `tools/`, and `THIRD_PARTY_NOTICES` at commit
> `32bdfcd8ce4e9e983f42deea222a9e1630e7ce5c` (v1.3.7). Everything above this line is the historical record of the
> decision as taken on 2025-11-02 and has not been rewritten.

### Implemented as decided

`SpatialIdAdapter` (`src/core/spatial/SpatialIdAdapter.js`) and `ZFXYConverter` (`src/core/spatial/ZFXYConverter.js`) exist as designed. `loadProvider()` still performs a dynamic `import('ouranos-gex-lib-for-javascript')` and sets `fallbackMode` on any failure; `getVoxelBounds()` still returns `{ zfxy, zfxyStr, vertices }` from the provider when available, from the built-in converter otherwise, and also when the provider throws mid-call. `DataProcessor._classifyBySpatialId` keys voxels by `zfxyStr` and carries the eight-vertex `bounds` on each record, which is what allows `RenderPlanner` to compute exact voxel centres in Spatial ID mode rather than interpolating them.

### Responsibility boundary — restated for clarity

| Component | Author | Notes |
|---|---|---|
| ZFXY encoding in the primary path — `Space`, `zfxy`, `zfxyStr`, `vertices3d()` | **Third party**: `ouranos-gex-lib-for-javascript`, MIT, Copyright (c) 2022 Geolonia Inc. | A reference implementation of the METI Spatial ID specification. Not authored by this project. |
| `SpatialIdAdapter` — provider abstraction, dynamic loading, fallback decision, vertex normalisation | This project | |
| Zoom selection — `calculateOptimalZoom`, `_calculateCellSizeAtZoom` | This project | Searches zoom 15–30 for the best match to a target cell size at a given latitude, within a tolerance. |
| Tile traversal — `neighbors`, `children`, `parent` | This project | `neighbors` wraps in X across ±180°; vertical subdivision is not modelled in `children`. |
| `ZFXYConverter` — built-in Web Mercator fallback | This project | Independent implementation; latitude clamped to ±85.0511287798°, fixed altitude binning. |
| Integration into voxel assignment and statistics | This project | `DataProcessor._classifyBySpatialId`, `SpatialIdQaMetrics`. |

Attribution is recorded in `THIRD_PARTY_NOTICES`, which reproduces the Geolonia Inc. copyright and MIT terms.

### How the dependency model evolved

This ADR specified the provider as a runtime dependency resolved by the consumer, with the library remaining functional when it is absent. The functional-without-it property still holds, but the packaging changed materially in **v1.3.7**:

- The provider is pinned by **commit**, not by tag or range: `ouranos-gex-lib-for-javascript` resolves to the source tarball for `5742889f20645fb0451c4870e5c54b5d34ab5c31`.
- It is a **devDependency**, not a runtime dependency. `tools/build-ouranos.js` builds the pinned source before bundling, and the result ships as a **lazily loaded chunk** in each of the ESM, CommonJS, and UMD outputs.
- Consumers therefore install nothing extra to get the official provider, and the dynamic `import()` in `loadProvider()` resolves to the bundled chunk rather than to a package the consumer must have installed. The only runtime dependency of the published library is `simple-statistics`.
- The package smoke test (`tools/package-smoke-test.js`) asserts the provider chunk is present in all three formats, so a build that would silently degrade every consumer to fallback mode cannot be published.

The `fallbackMode` path consequently changed in practical significance: it was the expected state for consumers who had not installed the provider, and it is now primarily a safety net for environments where the dynamic import or the provider itself fails.

### Related subsequent work

- **ADR-0015** added systematic edge-case QA for global coordinates, surfaced at runtime through `computeSpatialIdEdgeCaseMetrics(adapter)` and folded into `getStatistics()`.
- **ADR-0019** records the current architecture and the verified limitations of both Spatial ID paths — in particular that dateline handling exists for tile *neighbour* queries but that dataset-level antimeridian crossing is not supported, and that the fallback converter cannot represent polar latitudes.
