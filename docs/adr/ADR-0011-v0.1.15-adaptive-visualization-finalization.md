...
_detectOverlapAndRecommendMode(voxelInfo, voxelData) {
  ...
  if (overlapRisk > 0.5 && this.options.renderMode !== 'emulation-only') {
    return {
  ...
  return {
    recommendedMode: this.options.outlineRenderMode,
    recommendedInset: this.options.outlineInset
  };
}
```
...
- [ ] validation に `minOutlineWidth`/`maxOutlineWidth` → `outlineWidthRange` へのマッピングを追加
- [ ] `DEFAULT_OPTIONS.adaptiveParams.overlapDetection = false` を追加
- [ ] opacity range API のスケルトン実装（正規化/validation）と単体テストの下地を用意
- [ ] 優先順位/クランプ仕様のテストスケルトンを用意
- [ ] ドキュメント（API/README）に正規化仕様を反映

### Phase 1: パラメータ最適化（Week 1-2）
- [ ] 代表データセット（密集/疎/混在）での現行パラメータ評価
- [ ] A/Bテスト形式でのデフォルト値最適化
- [ ] プリセット係数の再調整と検証
- [ ] **マイルストーン**:
  - 代表3データセット（密集/疎/混在）で新デフォルト値が旧値より改善
  - 改善度合いをスクリーンショット比較とメトリクス表で記録
  - Phase 2 への Go/No-Go 判断を実施

### Phase 2: エッジケース対応（Week 3）
- [ ] Z軸スケール補正ロジックの実装とテスト
- [ ] 隣接重なり検出アルゴリズムの実装
- [ ] 自動モード切り替え推奨機能の実装
- [ ] **マイルストーン**: エッジケーステストスイートが100%パス

### Phase 3: UI/UX拡張（Week 4）
- [ ] パフォーマンスオーバーレイの実装
- [ ] 密度パターン生成機能の追加
- [ ] Advanced Examplesの拡充
- [ ] **マイルストーン**: examples/advancedでの新機能デモ完了

### Phase 4: ドキュメント・品質保証（Week 5）
- [ ] チューニング手順書の作成
- [ ] FAQ整備とトラブルシューティングガイド更新
- [ ] ベンチマーク拡張とメトリクス収集
- [ ] 統合テストと性能回帰テスト実行
- [ ] **マイルストーン**: v1.0.0移行準備完了
...
### 機能要件
- [ ] 密集・疎・混在の3パターンで、一律太さ/固定透明度より視認性が改善
- [ ] TopN/選択対象がデータに埋もれない（コントラスト比で定量評価）
- [ ] アウトライン描画モード（standard/inset/emulation-only）の切替が安定動作
- [ ] カスタム透明度resolver併用時の優先順位・クランプが一貫
- [ ] 優先順位（Resolver > Adaptive > Base）の仕様テストが通過（太さ/透明度）
- [ ] rangeクランプ（`outlineWidthRange`/`boxOpacityRange`/`outlineOpacityRange`）の境界値テストが通過
...
### 品質要件
- [ ] 既存テストケースが100%パス
- [ ] 新機能の単体テスト実装（境界値、エラーケース）
- [ ] Lint errors 0、JSDoc完備
- [ ] Validation 正規化（min/max→range、`overlapDetection` 既定値）のテストが通過

### ドキュメント要件
- [ ] チューニング手順が実データで検証済み
- [ ] FAQ項目が代表的な問題をカバー（最低10項目）
- [ ] examples/advancedで新機能の効果を確認可能
