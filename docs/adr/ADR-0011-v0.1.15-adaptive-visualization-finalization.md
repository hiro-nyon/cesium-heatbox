...
**Implementation State**: Implemented — the normalised range options `outlineWidthRange`, `boxOpacityRange`,
and `outlineOpacityRange` are current defaults in `src/utils/constants.js`.

> **Status note (2026-08-23)**: this ADR remained `Proposed` after the decision had been implemented and
> shipped, and the ADR index separately listed it as `In Progress` — a progress label, not a decision status.
> The status has been corrected to `Accepted` here and in the index; the decision text below is unchanged.
...
```javascript
// examples/observability/performance-overlay-demo.html
const overlayData = {
  rendering: {
