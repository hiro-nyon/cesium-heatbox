...
**Status**: Accepted  
**Implementation State**: Implemented — `layerStats` and `layerTop` are produced by both
`DataProcessor.classifyEntitiesIntoVoxels` and `_classifyBySpatialId`, and surfaced on voxel entity properties.  
**Date**: 2025-11-02  
**Author**: hiro-nyon  
...
> shipped in v0.1.18. The status has been corrected to `Accepted`; the decision text is unchanged.
...
src/
  ...
  utils/
    ...
    constants.js               # Update: Add DEFAULT_OPTIONS.aggregation
...
- [x] Add `aggregation` options to `constants.js` (default: `enabled: false`, include `topN`)
- [x] Add validation for `aggregation.enabled`, `byProperty`, `keyResolver`, `showInDescription`, `topN` in `validation.js`
- [x] Update JSDoc/TypeScript types for aggregation options

### Phase 2: DataProcessor Integration (Days 3-4) / フェーズ2：DataProcessor統合（3-4日）

- [x] Implement layer key resolution logic (`byProperty` and `keyResolver`)
- [x] Add `layerStats` Map to `voxelInfo` during entity classification
- [x] Calculate `layerTop` (most common layer) per voxel
- [x] Add error handling for invalid `keyResolver`
- [x] Resolve Cesium Property/PropertyBag via `getValue(now)` before key stringification
- [x] Support both uniform grid and spatial ID modes (同等の集計ロジック適用)
- [x] Normalize to `String(value ?? 'unknown')` and warn on anomalies

### Phase 3: Statistics Extension (Day 5) / フェーズ3：統計拡張（5日）

- [x] Aggregate `layerStats` across all voxels in `getStatistics()`
- [x] Return Top-N layers (default: 10) in `stats.layers`
- [x] Make Top-N configurable via `options.aggregation.topN`
- [x] Add unit tests for global layer aggregation

### Phase 4: Picking Integration (Day 6) / フェーズ4：ピッキング統合（6日）

- [x] Add `properties.layerTop` to rendered voxel entities
- [x] Optionally add layer breakdown to voxel description
- [x] Add flag `aggregation.showInDescription` (default: `true`)
- [x] Escape layer keys in description with `escapeHtml` (XSS対策)

### Phase 5: Testing (Days 7-8) / フェーズ5：テスト（7-8日）

- [x] Unit tests: `byProperty` aggregation
- [x] Unit tests: `keyResolver` aggregation
- [x] Unit tests: Error handling (invalid resolver, non-string keys)
- [x] Unit tests: Top-N summary
- [x] Integration tests: Full workflow with aggregation enabled
- [x] Performance tests: Verify ≤ +10% memory/time overhead

### Phase 6: Documentation & Examples (Days 9-10) / フェーズ6：ドキュメント・例（9-10日）

- [x] Update README with aggregation section
- [x] Add example: urban analytics (building types)
- [x] Add example: multi-source data (sensor coverage)
- [x] Document best practices (categorical keys, avoid timestamps)
- [x] Update API documentation (JSDoc)
