...
- [ ] Add `aggregation` options to `constants.js` (default: `enabled: false`, include `topN`)
- [ ] Add validation for `aggregation.enabled`, `byProperty`, `keyResolver`, `showInDescription`, `topN` in `validation.js`
...
- [ ] Resolve Cesium Property/PropertyBag via `getValue(now)` before key stringification
- [ ] Support both uniform grid and spatial ID modes (同等の集計ロジック適用)
- [ ] Normalize to `String(value ?? 'unknown')` and warn on anomalies
...
- [ ] Make Top-N configurable via `options.aggregation.topN`
...
- [ ] Escape layer keys in description with `escapeHtml` (XSS対策)
