...
**Status**: Accepted  
**Implementation State**: Implemented — `quantile` and `jenks` schemes, `classificationTargets` for colour,
opacity, and width, and the `Legend` UI (`src/ui/Legend.js`) are all current. Support for explicit
pre-computed breaks was added later so temporal global scope can fix a domain across slices.  
**Date**: 2025-11-18  
**Author**: hiro-nyon  
...
> shipped in v1.1.0. The status has been corrected to `Accepted`; the decision text is unchanged. The
> historical `Approval: Pending review` footer below records the state at authoring time, not the current status.
...
**Approval at authoring time**: Pending review
**Retrospective status (2026-08-23)**: Accepted after implementation and release verification
