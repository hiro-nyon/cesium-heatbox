...
v1.1.0 では、**Resolver の機能を `adaptiveParams.*Range` + `classification.targets` で完全に再現**し、段階的な移行パスを提供する必要があります。
 ...
 * @property {ClassificationTargets} [targets] - 分類対象の視覚変数
 * @property {number} [classes=5] - クラス数
 * @property {number[]} [thresholds] - threshold 用の境界配列
 * @property {string[]} [colorMap] - 色配列
 * @property {number[]} [opacityRange=[0, 1]] - 透明度の範囲（NEW）
 * @property {number[]} [widthRange=[1, 5]] - 太さの範囲（NEW）
 */
```
...
const heatbox = new Heatbox(viewer, {
  classification: {
    enabled: true,
    scheme: 'log',
    targets: { color: true }  // または省略時のデフォルト
  }
});
```
...
case 'jenks':
  ...
  const backend = getBackend();
  const clusters = backend.ckmeans(values, classes);
...
```javascript
// src/core/adaptive/AdaptiveController.js（イメージ）

class AdaptiveController {
  calculateAdaptiveParams(voxelInfo, classifier, options) {
    ...
    const targets = options.classification?.targets || { color: true };
    const result = {};
    ...
    // 透明度
    if (targets.opacity) {
      const [minOpacity, maxOpacity] = options.adaptiveParams?.boxOpacityRange || [0, 1];
    ...
    // 太さ
    if (targets.width) {
      const [minWidth, maxWidth] = options.adaptiveParams?.outlineWidthRange || [1, 5];
      result.outlineWidth = minWidth + normalized * (maxWidth - minWidth);
      
      // TopN ブーストとの合成
      if (voxelInfo.isTopN && options.highlightTopN) {
        result.outlineWidth += options.highlightTopNWidthBoost || 2;
      }
    }
...
1. ユーザーがオプション指定
   ...
   { 
     classification: { 
       scheme: 'quantile', 
       targets: { color: true, opacity: true, width: true },
       opacityRange: [0.3, 1.0],
       widthRange: [1, 5]
     },
     adaptiveParams: {
...
6. GeometryRenderer.createVoxelBox()
   ...
   ↓
7. Legend.update()
   - classifier.breaks と targets から凡例を生成
```
...
getStatistics() {
  return {
    ...
    classification: {
      ...
      // v1.1.0: 拡張
      targets: this.options.classification?.targets || { color: true },
 ...
 * @param {number[]} values - データ配列
 * @param {number} classes - クラス数
 * @returns {{ breaks: number[], normalize: Function, clusters: number[][] }}
 */
function createJenksClassifier(values, classes) {
  ...
  return { breaks, normalize, classify, clusters };
}
```
...
class AdaptiveController {
   ...
   * @param {Object} classifier - 分類コンテキスト
...
- [ ] `src/utils/classification.js` の `createClassifier` を更新
  - `targets` オプションの受け入れ
  - `opacityRange` / `widthRange` のサポート
- [ ] 単体テスト: `test/utils/classification.test.js` に targets のテストを追加
...
describe('classification - targets', () => {
  it('should apply classification to color only by default', () => {
    const classifier = createClassifier({
      scheme: 'linear',
      domain: [0, 100],
      targets: { color: true }  // デフォルト
    });
    
    expect(classifier.targets).toEqual({ color: true });
  });
  
  it('should support opacity and width targets', () => {
    const classifier = createClassifier({
      scheme: 'log',
      domain: [1, 100],
      targets: {
    ...
    expect(classifier.targets.opacity).toBe(true);
    expect(classifier.targets.width).toBe(true);
    expect(classifier.opacityRange).toEqual([0.3, 1.0]);
    expect(classifier.widthRange).toEqual([1, 5]);
  });
});
```
  ...
  it('should apply jenks classification with natural breaks', async () => {
    ...
    heatbox = new Heatbox(viewer, {
      classification: {
        ...
        scheme: 'jenks',
        classes: 3,
        targets: { color: true, opacity: true }
      }
    });
...
#### 1. API の複雑化
- `classificationTargets`, `opacityRange`, `widthRange` など新オプションの追加
- ユーザーの学習コストが増加
...
**Mitigation**:
- simple-statistics の ckmeans は O(n² k) の計算量
...
function createJenksClassifier(values, classes) {
  ...
  _cachedBreaks = { breaks, normalize, classify, clusters };
