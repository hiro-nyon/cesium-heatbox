...
**Status**: Accepted
**Decision date / 決定日**: 2026-04-06 (v1.3.0 / v1.3.4)
**Recorded / 記録日**: 2026-08-23
**Author**: hiro-nyon
**Target Version**: v1.3.0
**Implementation State**: Implemented
**Related**: ADR-0009 (VoxelRenderer responsibility separation), ADR-0019 (current runtime architecture), ROADMAP v1.3.0
**Primary commits**: `9feb120` (v1.3.0 performance foundation), `6d669a9` (v1.3.4 camera-refresh and planner recovery fixes), `3aa8e86` (v1.3.7-alpha thick-frame reclamation fix)

> **Retrospective record / 後追い記録である旨.** This ADR was **written on 2026-08-23**, after the fact. The decision it records was taken and shipped in v1.3.0–v1.3.4 (April 2026) but was never captured as an ADR at the time; this document reconstructs it from Git history, CHANGELOG, ROADMAP, and the source at `32bdfcd`. The `Decision date` field is when the decision was made, not when this file was authored.
>
> 本ADRは **2026-08-23 に後から作成した** 記録である。記録対象の決定は v1.3.0〜v1.3.4（2026年4月）に実施・リリースされたが、当時ADRとして記録されなかった。本文書は Git history、CHANGELOG、ROADMAP、および `32bdfcd` 時点のソースから再構成したものである。`Decision date` は決定時点を指し、本ファイルの作成日ではない。
...
3. **Unbounded work per pass.** Selection (`VoxelSelector`) chose *which* voxels were eligible, but nothing reduced the set based on what the camera could actually see, and nothing prioritised near voxels over far ones. The render budget was a static option (`maxRenderVoxels`), independent of viewing conditions.
   **パスごとの作業量が無制限。** `VoxelSelector` は描画*対象候補*を選ぶが、カメラから実際に見えるかどうかで絞り込む機構はなく、近いボクセルを優先する機構もなかった。描画予算は視点条件と無関係な静的オプション（`maxRenderVoxels`）だった。
...
|---|---|---|
| `VoxelSelector` (ADR-0009) | Which voxels are *eligible*, given a limit and a strategy?<br>上限と戦略のもとで、どのボクセルが*候補*か | None — pure<br>なし（純粋関数） |
...
`RenderPlanner` uses exactly three camera-derived inputs. Each is listed here with the code that consumes it, so the claim can be checked.

`RenderPlanner` が使うカメラ由来の入力は正確に3種類である。検証可能なように、各項目を消費側コードとともに示す。
...
with ties broken by raw `count`. The proximity bonus is `max(0, 100_000 - distance)` where `distance` is computed from `camera.positionCartographic` to an estimated voxel centre using a local metres-per-degree approximation. TopN membership therefore dominates density, and density dominates proximity; proximity only orders voxels of equal density.

同点は生の `count` で決着する。近接ボーナスは `max(0, 100_000 - distance)` で、`distance` は `camera.positionCartographic` から推定ボクセル中心までを局所的な「度→メートル」近似で算出する。したがって TopN 所属が密度を上回り、密度が近接を上回る。近接は密度が同一のボクセル間の順序付けにしか効かない。
...
**(c) Angular visibility filter — `_cullByViewport`.** For each voxel the planner computes the unit vector from `camera.position` to the estimated voxel centre and takes its dot product with `camera.direction`. Voxels with a non-positive dot product — behind the camera — are dropped. The remaining voxels are kept if the angle to the view direction is within a horizontal allowance derived from the field of view and aspect ratio:

**(c) 角度による可視性フィルタ — `_cullByViewport`。** 各ボクセルについて `camera.position` から推定中心への単位ベクトルを求め、`camera.direction` との内積を取る。内積が非正のボクセル（カメラ背面）は除外する。残りは、視線方向との角度が視野角とアスペクト比から導いた水平許容範囲内であれば保持する。
...
`aspectRatio` comes from `camera.frustum.aspectRatio`, falling back to the canvas client dimensions. If camera position, direction, FOV, or canvas is unavailable, the filter is skipped entirely and all voxels are returned — the planner degrades to a no-op rather than hiding data.

`aspectRatio` は `camera.frustum.aspectRatio` から取得し、無ければ canvas の client 寸法にフォールバックする。カメラ位置・方向・FOV・canvas のいずれかが取得できない場合、フィルタは完全にスキップされ全ボクセルが返る — planner はデータを隠すのではなく no-op に退避する。
...
|---|---|
| Frustum-plane culling<br>視錐台平面カリング | A single angular cone test against `camera.direction`, using the horizontal half-FOV. There are no near/far plane tests and no separate vertical bound.<br>`camera.direction` に対する単一の円錐角度判定（水平半FOV使用）。near/far 平面判定も独立した垂直方向の境界も無い |
...
The correct description of the mechanism is **camera-aware prioritisation with a heuristic angular filter and a height-scaled budget**. It reduces work under common viewing conditions; it does not compute exact visibility.

この機構の正確な記述は **ヒューリスティックな角度フィルタと高度スケール予算を伴うカメラ考慮の優先順位付け** である。一般的な視点条件下で作業量を削減するが、正確な可視性を計算するものではない。

## Consequences / 影響

### Benefits / 利点

- A render pass that changes only voxel values mutates existing box entities rather than replacing the collection, removing the dominant source of heap growth across repeated render/clear cycles.
  ボクセル値のみが変わる描画パスは、コレクションを置換せず既存 box entity を書き換える。描画/クリア反復におけるヒープ増加の主要因が除去される。
...
- Planning is testable without Cesium rendering. `RenderPlanner` takes a viewer-shaped object and returns plain data, so its heuristics are unit-tested directly (`test/core/render/RenderPlanner.test.js` covers behind-camera culling, budget reduction at high altitude, and the guarantee that the budget is never raised above a small `maxRenderVoxels`).
  Cesium 描画なしで計画をテストできる。`RenderPlanner` は viewer 形状のオブジェクトを受けてプレーンなデータを返すため、ヒューリスティックを直接ユニットテストできる（`test/core/render/RenderPlanner.test.js` がカメラ背面のculling、高高度での予算縮小、小さい `maxRenderVoxels` を超えて予算が増えないことを検証）。
...
- Camera-aware budgeting gives distant, wide views a cheaper frame without the caller having to retune `maxRenderVoxels` per viewpoint.
  カメラ考慮の予算制御により、遠方の広域ビューでは呼び出し側が視点ごとに `maxRenderVoxels` を調整しなくてもフレームが軽くなる。
...
Read the three `SYNC` edges as the per-key decision: a key seen for the first time is an ADD, a key already present is an in-place UPDATE, and inset/polyline geometry is recreated on both paths. `endFrame` turns *absence from the desired set* into a REMOVE — that is the only deletion path in a normal frame.

**日本語での読み方**: `SYNC` から出る3本の辺が key ごとの判断を表す。初出の key は ADD、既存の key はその場での UPDATE、インセット/ポリラインのジオメトリはどちらの経路でも再生成される。`endFrame` は *目標集合に存在しないこと* を REMOVE に変換する — 通常フレームにおける唯一の削除経路である。
...
**C. Fold planning into `VoxelSelector`.** One fewer component. Rejected: `VoxelSelector` is documented and implemented as Cesium-free and pure ("純粋関数として設計（Cesium依存なし）"); reading `viewer.camera` inside it would break that property and make selection untestable without a viewer mock. Keeping a separate Cesium-reading planner preserves a pure selection layer.

**C. 計画機能を `VoxelSelector` に統合する。** コンポーネントが1つ減る。却下理由: `VoxelSelector` は「純粋関数として設計（Cesium依存なし）」と文書化され、そのように実装されている。内部で `viewer.camera` を読めばこの性質が壊れ、viewer mock なしに選択をテストできなくなる。Cesium を読む planner を別に置くことで、純粋な選択層が保たれる。
