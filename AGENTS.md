...
- If a PR is required, use the GitHub CLI (`gh pr create --base next --head <branch>`) to open it once the branch is ready. Stable promotion uses a separate reviewed PR from `next` to `main`.
